...
Make sure that files in r776-a767_distance and ke_distance 
...
def get_x_y_z(xall, yall, nbins=1000, weights=None, kT=1.0, range=[[0,20], [0,15]]):
...
# obtain mesh data
wt_x, wt_y, wt_z = get_data('../../fig5a/ke_distance/WT_EGFR_ke.txt',
                            '../r776-a767_distance/WT_EGFR_capping.txt')

g_x, g_y, g_z = get_data('../../fig5a/ke_distance/D770_N771insG_ke.txt',
                         '../r776-a767_distance/D770_N771insG_capping.txt')

gy_x, gy_y, gy_z = get_data('../../fig5a/ke_distance/D770_GY_ke.txt',
                            '../r776-a767_distance/D770_GY_capping.txt')

n_x, n_y, n_z = get_data('../../fig5a/ke_distance/N771_P772insN_ke.txt',
                         '../r776-a767_distance/N771_P772insN_capping.txt')
...
plt.subplot(2, 2, 1)
wt_ax = plt.gca()
wt_ax.set_xlim(left=0.0, right=20)
wt_ax.set_ylim(top=15, bottom=0)
...
plt.subplot(2, 2, 4)
n_ax = plt.gca()
n_ax.set_xlim(left=0.0, right=20)
n_ax.set_ylim(top=15, bottom=0)
...
plt.subplot(2, 2, 3)
gy_ax = plt.gca()
gy_ax.set_xlim(left=0.0, right=20)
gy_ax.set_ylim(top=15, bottom=0)
...
plt.subplot(2, 2, 2)
g_ax = plt.gca()
g_ax.set_xlim(left=0.0, right=20)
g_ax.set_ylim(top=15, bottom=0)
